<template>
  <div class="min-h-screen bg-gray-100 pt-28 md:pt-36 pb-24 flex flex-col items-center">
    <h3 class="text-lg font-bold text-[#27628C] mb-10 text-center">
    ...
    <div v-if="!generatedUrl" class="flex flex-col items-center space-y-6">
      ...
      >
        ...
        <div class="absolute bottom-2 inset-x-0 flex flex-row items-center justify-center space-x-4">
          <div
            class="w-20 h-20 rounded-full bg-gray-200 border-2 border-[#27628C] overflow-hidden flex items-center justify-center cursor-pointer"
...
const validateForm = () => {
  ...
  if (!testimonial.value.trim()) newErrors.testimonial = "Please enter your testimonial.";
  if (!authorImage.value) newErrors.global = "Please upload your image.";
  errors.value = newErrors;
  return Object.keys(newErrors).length === 0;
};

watch(authorName, () => {
  if (errors.value.name) setTimeout(() => (errors.value.name = undefined), 1000);
});
watch(testimonial, () => {
  if (errors.value.testimonial) setTimeout(() => (errors.value.testimonial = undefined), 1000);
});
watch(authorImage, () => {
  if (errors.value.global) setTimeout(() => (errors.value.global = undefined), 1000);
});
...
function downloadImage() {
  ...
  a.download = "medlabconvo-testimonial.png";
  a.click();
}
